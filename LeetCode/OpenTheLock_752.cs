...
namespace LeetCode
{
	class OpenTheLock_752
	{
		...
		//超过时间限制，没什么办法，换Python写了
		public int OpenLock(string[] deadends, string target)
		{
			var dead = new List<string>(deadends);
			if (dead.Contains("0000")) return -1;
			if (dead.Contains(target)) return -1;
			if (IfCould(dead, target)) return -1;
			var bfs = new List<string> { "0000" };
			var exis = new List<string> { "0000" };
			int count = 0;
			...
			while (bfs.Count != 0)
			{
				...
				for(int j = 0; j < cn; j++)
				{
					...
					for (int i = 0; i < 4; i++)
					{
						string up = UpdateLock(cur, i, 1);
						string down = UpdateLock(cur, i, -1);
						if (up == target || down == target) return count;
						if (!exis.Contains(up) && !dead.Contains(up))
						{
							bfs.Add(up);
							exis.Add(up);
						}
						if (!exis.Contains(down) && !dead.Contains(down))
						{
							bfs.Add(down);
							exis.Add(down);
						}
