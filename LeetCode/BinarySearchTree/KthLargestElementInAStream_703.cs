...
namespace LeetCode.BinarySearchTree
{
	class KthLargest
	{
		...
		{
			this.k = k;
			heap = new BinaryHeap<int>(k);
			for(int i = 0; i < nums.Length; i++)
			{
				if (heap.mCount == k)
				{
					int temp = heap.Dequeue();
					if (temp < nums[i]) heap.Enqueue(nums[i]);
					else heap.Enqueue(temp);
				}
				else heap.Enqueue(nums[i]);
			}
		}
		...
		public int Add(int val)
		{
			...
			{
				int temp = heap.Dequeue();
				if (temp < val) heap.Enqueue(val);
				else heap.Enqueue(temp);
			}
			else heap.Enqueue(val);
